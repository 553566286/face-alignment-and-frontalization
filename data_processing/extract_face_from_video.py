...
def deal_with_filename(subdir):
    ...
    #           imgs_path = os.path.join(subdir,each_frame,each_class,each_sample)
    #               os.mkdir(os.path.join(targetsub,each_class,each_sample))
    S = subdir.split('/')
    the_class = S[9]
    sample_name = S[10]
    for filename1 in os.listdir(subdir):
        frame = cv2.imread(os.path.join(subdir,filename1))
...
#      # /media/ustb/Dataset2/DEIC DATA/data/Annotated_data_Sub01_Trial01/Sub01_Trial01_Sweep01_Label0/RGB
# def deal_with_subfiles(subdir1):
#     for .listdir(subdir1):
#         subdir2 = os.path.join(subdir1,subdir22)
#         subdir = os.path.join(subdir2,'RGB')
#         deal_with_filename(subdir)
#
#
with concurrent.futures.ProcessPoolExecutor() as executor:
    subfiles = '/media/ustb/Dataset2/biovid/PartA/reprocess_slstm/B_frames_arg'
    for each_start_frame in os.listdir(subfiles):
        ...
        for each_class in  os.listdir(frame_sub):
            ...
            for each_file in os.listdir(class_sub):
                subfiles = glob.glob(os.path.join(class_sub,each_file))
